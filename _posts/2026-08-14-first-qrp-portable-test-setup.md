...
**Two settings that decide whether the station runs by itself.** Auto Seq should stay on. It is what steps the messages through a QSO, and it is what lets WSJT-X keep calling CQ between contacts without a click. The setting that trips people up is "Disable Tx after sending 73" on the General tab: leave that checked and transmission simply stops after every contact, so each new CQ has to be started by hand. Unchecked, WSJT-X moves on to the CQ message and carries on. Worth knowing alongside it: the Tx Watchdog, six minutes by default, deliberately stops transmission if the operator has not touched anything for that long. If CQ calling dies during a quiet spell, that is usually why, and it is a safety feature rather than a fault.

There is one exception to leaving Auto Seq alone. If you want to send a custom free-text message instead of the standard 73, say a WWFF reference for a hunter who asked for it on air, Auto Seq will overwrite whatever you typed on the next decode. The only way through is to type it when the QSO has actually reached the 73 stage, untick Auto Seq for that single transmission, let it go out, and tick it straight back on. Forgetting that last step is the classic error, because sequencing then stops working for the rest of the session without saying so.
